...
def main(kerkesa1,addr):
    ...
    if(kerkesa == False):
       ...
       return BASHKETINGELLORE(kerkesa[1])
        ...
        return "Emri i Hostit: "+EMRIIKOMPJUTERIT()
    elif(kerkesa[0] == "NUMRIIPORTIT"):
        return "Porti i klientit: "+str(NUMRIIPORTIT(addr))
    elif(kerkesa[0] == "IPADRESA"):
        return "Ip addresa e klientit: " + IPADRESA(addr)
...
def NdegjoKlientin(client,address):
    KeepConnection = False
    while True: #kjo mundson qe kumunikimi me klientin te mos nderprehet
        data = client.recv(1024)
        data = data.decode()
        #Nese klienti nuk ka kerkuar qi lidhja te mbahet e hapur per kerkesa tjera
        if(data == "Mbaje Lidhjen"):
            KeepConnection = True;
            continue
        #Perndryshe proceso dhe mbyll lidhjen
        if(not KeepConnection):
            res = str(main(data,address))
...
while True:
        ...
        start_new_thread(NdegjoKlientin, (client,address) )
